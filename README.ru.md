...
- **Поиск** - переход к любому периоду в прошлом, не только
  "недавнему"
- **Шесть цветовых тем** - светлая/тёмная, или автоматически по
  времени суток
...
- **Сброс сортировки** - один клик отменяет случайную сортировку по
  клику на столбец
- **Дерево папок** - Alt+Enter на `! menu` показывает, как реально
...
Все десять пунктов панели уже поставляются с собственной подходящей
...
Полный список всех десяти ключей - в справочнике настроек → Пути.
...
Alt+Enter на `! menu` открывает отдельное небольшое окно (тот же
...
в виде дерева - построенного исключительно из настроенных путей, а не
из того, что уже записано.
...
| `Language` | Язык интерфейса, соответствует секции в `RecentTab_lang.ini` |
...
| `RecIcon` / `ResetIcon` / `RefreshIcon` / `MenuIcon` / `AgeIcon` / `BackIcon` / `AutoRefreshIcon` / `SortResetIcon` / `LostIcon` / `FallbackIcon` / `DeadPathIcon` | Замена иконок по отдельным пунктам, всё необязательно |
...
- `RecentTab_example.ini` - справочная копия всех доступных настроек,
  ...
  новое.
- `RecentTab_lang.ini` - текст интерфейса; должен оставаться в той же
  папке, что и файл плагина, так как ищется относительно его
  собственного расположения, а не папки настроек TC
