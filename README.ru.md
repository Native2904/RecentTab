...
[English](README.md) · [Deutsch](LIESMICH.md) · **Русский** · [Українська](README.uk.md) · [Dansk](README.da.md)

Плагин файловой системы (WFX) для Total Commander, который показывает,
над чем вы недавно работали: недавно изменённые и новые файлы из
нужных вам папок, собранные в одной виртуальной панели.
...
- **Запись** - простой переключатель вкл/выкл, переживает
  перезапуски, собственный счётчик времени
- **Поиск** - переход к любому периоду в прошлом, не только
  "недавнему"
- **Цветовые темы** - четыре готовые плюс своя, каждая светлая/тёмная,
  или автоматически по времени суток
- **Автообновление** - панель обновляется сама по расписанию, без
  ручного `Strg+R`
- **Потерянные файлы** - замечает, когда отслеживаемый файл исчезает,
  и может найти, куда он делся
- **Дополнительные столбцы** - тип изменения, относительное время,
  папка-источник, номер сессии, уже открыт, статус блокировки
- **Регуляторы яркости** - подстройка любой темы без выбора новых
  цветов
- **Встроенные шрифты** - JetBrains Mono и Fira Code идут в комплекте,
  отдельная установка не нужна
- **Свои иконки** - иконка любого пункта панели заменяема
- **Сброс сортировки** - один клик отменяет случайную сортировку по
  клику на столбец
- **Дерево папок** - `! Дерево папок` в `! menu` (или Alt+Enter на
  `! menu`) показывает, как реально
  ветвятся отслеживаемые папки, мёртвые пути отмечены явно

Этот список растёт вместе с плагином - как именно работает каждая из
этих функций, описано ниже в разделе "Расширенная настройка", точные
ключи ini - в справочнике настроек / справочнике столбцов далее.
...
- Нет диалога настроек: настройка происходит через прямое
  редактирование `RecentTab.ini` (сами файлы в списке можно
...
Language=auto        ; auto (по умолчанию) = язык Total Commander / или фиксированно: eng / deu / rus / ...
...
RootButtons=          ; вынести Reset;Refresh;Search;AutoRefresh;SortReset;FolderTree на верхний уровень, а не только внутрь "! menu"
...
везде обычные системные цвета (диалог Alt+Enter, окно поиска, окно
сведений о потерянном файле, дерево папок). Каждая роль, которая несёт смысл через
...
По умолчанию RecentTab просто использует язык Total Commander - тот,
что выбран в Конфигурация → Настройка → Язык (`WCMD_RUS.LNG` →
русский, `WCMD_DEU.LNG` → немецкий, встроенный английский TC →
английский). `Language=rus` (или любое другое имя секции) в
`[Settings]` задаёт язык независимо от TC. Язык считывается один раз
за сеанс TC - после смены языка в TC перезапустите TC.

Тексты берутся из `RecentTab.lng` - один файл, по секции
`[eng]`/`[deu]`/`[rus]` на язык, как принято у плагинов TC - и
читаются собственным UTF-8-безопасным парсером плагина, а не через
`GetPrivateProfileString` Windows (которая портит нелатинские алфавиты
через ANSI-кодовую страницу системы, если у файла нет BOM UTF-16LE).
Поставляется с `eng`, `deu`, `rus`, `ukr` и `dan`; любой пропущенный или пустой ключ
автоматически возвращается к английскому. (До версии 1.5.1 файл
назывался `RecentTab_lang.ini` - он по-прежнему читается, если
`RecentTab.lng` нет.)

Добавить язык: скопируйте секцию `[eng]`, переименуйте её в
трёхбуквенный код TC (часть после `WCMD_` в имени языкового файла TC)
и переведите значения. Подстановки вроде `%ls`/`%d`/`%lld` должны
остаться теми же и в том же порядке - если в строке они отличаются,
вместо неё показывается английский вариант, чтобы не рисковать сбоем.
Ключи с окончаниями `_one`/`_few`/`_many` - формы единственного и
множественного числа (для русского: 1 минуту / 2 минуты / 5 минут).

Побочный эффект, о котором стоит знать: записи `! ...` на панели тоже
переводятся. Кнопка или горячая клавиша TC, которая обращается к одной
из них по имени (например, `\\RecentTab\! ЗАП`), работает только на
этом языке - если вы такими пользуетесь, задайте `Language=` явно.
...
Все одиннадцать пунктов панели уже поставляются с собственной подходящей
иконкой как активным значением по умолчанию (см. `icons\` и раздел
`[Icons]` файла `RecentTab_example.ini`) - единое семейство
"оранжевое кольцо, синий символ", каждая форма отражает, что делает её
пункт. Любую можно просто переопределить в `[Icons]`, указав вместо
неё свой `.ico`/`.exe`/`.dll` - относительный путь вроде
`icons\age.ico` разрешается относительно папки этого ini-файла:
...
Полный список всех одиннадцати ключей - в справочнике настроек → Пути.
...
полями, **ОТ** и **ДО** - ввод более гибкий, чем кажется:
...
- Время не указано? ОТ по умолчанию - начало дня, ДО - конец дня -
  так что дата без времени в ДО охватывает весь день.
- Оставьте ДО пустым для варианта "**до сейчас**".
...
### Дерево папок - увидеть, как реально ветвятся отслеживаемые папки

`! menu` → `! Дерево папок` (или Alt+Enter прямо на `! menu`) открывает
отдельное небольшое окно (тот же
визуальный стиль, что и окно поиска - тонкие рамки, своя тема, полностью
независимо от вида самого TC), показывающее каждую отслеживаемую папку
в виде дерева - вместе с папками, в которых на самом деле лежат
записанные файлы, ведь обычно они находятся на несколько уровней
глубже самой отслеживаемой папки.

Папки, у которых общий родитель, автоматически группируются под ним,
даже если сам этот родитель не отслеживается - просто визуальная
группировка, ничего дополнительно не отслеживается. Цепочка папок, у
каждой из которых ровно один потомок - без реального ветвления по
пути - сворачивается в одну строку вместо пяти-шести отступов, не
несущих дополнительной информации; если файлы всё же есть
непосредственно в одной из таких промежуточных папок, счётчик остаётся
виден именно у её имени, а не скрывается сворачиванием.

Отслеживаемая папка, путь которой больше не существует на диске,
получает собственный, намеренно бросающийся в глаза значок (дорожное
заграждение, а не тот же призрак, что используется для отдельных
потерянных файлов в другом месте - целая пропавшая настроенная папка
это другая ситуация, чем исчезновение одного файла).

Высота окна ограничена примерно высотой стандартного окна Lister (TC не
даёт плагинам способа узнать его реальный размер, так что это лишь
приближение, а не настоящее измерение) - всё, что выше, прокручивается
колесом мыши. См. `notes/folder-tree/` для более полного обоснования,
включая то, зачем именно для этого понадобился GDI+.

## Справочник настроек

Все существующие настройки в одном месте - сгруппированы точно так
же, как сам файл `RecentTab_example.ini` (переключатели функций,
цвета, пути), чтобы этот список и файл всегда совпадали. Разделы выше
объясняют *почему*; здесь - *что*, намеренно кратко. Новые настройки
получают одну строку здесь, больше ничего не переписывается.

**Переключатели функций**

| Настройка | Что делает |
|---|---|
| `ConfirmReset` | Спрашивать "точно сбросить?" перед очисткой истории |
| `SortDescending` | Сначала новые (1) или сначала старые (0) |
| `DebugLogging` | Писать `RecentTab_debug.log` - выключить перед передачей сборки |
| `Language` | `auto` (по умолчанию) - язык TC; либо фиксированное имя секции из `RecentTab.lng` |
| `IncludeDefaultFolders` | Добавлять свои блоки `[Watched:...]` к шести папкам по умолчанию вместо замены |
| `RootButtons` | Какие служебные пункты стоят на верхнем уровне вместо `! menu` |
| `UseSearch` | 0 полностью скрывает `! Search` |
| `ShowLiveClock` | Тикающие часы в заголовке окна Alt+Enter |
| `OnlyExtensions` | Глобальный список разрешённых расширений - отменяет любое исключение, пока установлен |
| `ShowChangeType` / `ShowRelativeTime` / `ShowSourceFolder` / `ShowSession` / `ShowOpened` / `ShowLocked` | Шесть дополнительных столбцов - см. справочник столбцов ниже |
| `OpenedTracking` | Память `session` или `permanent` для `ShowOpened` |
| `AllowDriveNetwork` / `AllowDriveRemovable` / `AllowDriveCDRom` | Разрешить `ShowLocked` проверять и эти типы дисков |
| `NoColors` | Везде обычные системные цвета, без темы |
...
| `LostFilesTracking` | Память `session` или `permanent` для списка потерянных |
| `LostSearchStrict` | Требовать точное совпадение размера и даты для подтверждения находки в "Search again" |
...
| `Mode` | `dark` или `light` |
| `TimeBasedMode` | Переключать Mode автоматически по времени вместо этого |
| `LightStartHour` / `DarkStartHour` | Время переключения, если TimeBasedMode=1 |
| `Background` / `Foreground` / `Heading` / `Green` / `Accent2` / `Yellow` / `Accent4` / `Muted` | Восемь цветовых ролей, используются только при `Name=custom` |

**Пути** (специфично для машины - не предназначено для передачи/копирования как есть)

| Настройка | Что делает |
|---|---|
| `Path` (внутри `[Watched:Name]`) | Сама папка |
| `Exclude` | Подпути, пропускаемые в этой папке |
| `ExcludeExtensions` | Расширения, пропускаемые в этой папке |
| `RecIcon` / `ResetIcon` / `RefreshIcon` / `MenuIcon` / `AgeIcon` / `BackIcon` / `AutoRefreshIcon` / `SortResetIcon` / `FolderTreeIcon` / `LostIcon` / `FallbackIcon` / `DeadPathIcon` | Замена иконок по отдельным пунктам, всё необязательно |

## Справочник столбцов

Все шесть находятся в `RecentTab.ini`, по умолчанию выключены,
добавляются на панель через собственный диалог TC Shift+F1 "Configure
custom columns".

| Столбец | Настройка | Работает и внутри `! Lost` |
|---|---|---|
| Изменён / Новый | `ShowChangeType=1` | Да |
| Относительное время | `ShowRelativeTime=1` | Да |
| Папка-источник | `ShowSourceFolder=1` | Да |
| Номер сессии | `ShowSession=1` | Да |
| Уже открыт | `ShowOpened=1` | Да |
| Статус блокировки | `ShowLocked=1` | Нет - файл уже известно, что исчез, проверка блокировки всегда выдала бы только "Не найден" |
...
- `RecentTab_example.ini` - справочная копия всех доступных настроек,
  намеренно поставляется под этим именем (не `RecentTab.ini`): так
  распаковка обновления никогда не перезапишет вашу собственную
  конфигурацию. Скопируйте этот файл, переименуйте копию в
  `RecentTab.ini`, и редактируйте именно копию - плагин ищет файл
  именно с таким именем, но нормально работает с разумными значениями
  по умолчанию, даже если этого файла ещё вообще не существует. После
  обновления сравните этот файл со своим `RecentTab.ini`, чтобы найти
  новое.
- `RecentTab.lng` - текст интерфейса на всех поставляемых языках;
  должен оставаться в той же папке, что и файл плагина, так как ищется
  относительно его собственного расположения, а не папки настроек TC
...
Только при первой установке: скопируйте `RecentTab_example.ini`,
переименуйте копию в `RecentTab.ini`, в той же папке. Всё работает и
без этого шага, с чистыми значениями по умолчанию - просто тогда
нечего будет редактировать, если захочется что-то изменить.
