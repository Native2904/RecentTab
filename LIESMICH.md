...
[English](README.md) · **Deutsch** · [Русский](README.ru.md) · [Українська](README.uk.md) · [Dansk](README.da.md)

Ein Total-Commander-Dateisystem-Plugin (WFX), das zeigt, woran du
zuletzt gearbeitet hast: kürzlich geänderte und neu angelegte Dateien
aus den Ordnern, die dich interessieren, gesammelt in einem virtuellen
Panel.
...
- **Suche** - in jeden vergangenen Zeitraum springen, nicht nur
  "kürzlich"
- **Farbschemata** - vier Vorlagen plus ein eigenes, jeweils hell/dunkel,
  oder automatisch nach Uhrzeit
...
- **Ordnerbaum** - `! Ordnerbaum` im `! menu` (oder Alt+Enter auf
  `! menu`) zeigt, wie sich die
  beobachteten Ordner tatsächlich verzweigen, tote Pfade klar markiert
...
- Kein Einstellungsdialog: Konfiguration erfolgt durch direktes
  Bearbeiten von `RecentTab.ini` (die Dateien in der Liste selbst sind
  ganz normal bearbeitbar).
...
Language=auto        ; auto (Standard) = Sprache von Total Commander übernehmen / oder fest: eng / deu / rus / ...
...
RootButtons=          ; Reset;Refresh;Search;AutoRefresh;SortReset;FolderTree an die Wurzel holen, statt nur im "! menu"
...
Info-Fenster für verlorene Dateien, Ordnerbaum). Jede Rolle, die über Farbe
...
Standardmäßig spricht RecentTab einfach die Sprache von Total
Commander: die, die unter Konfiguration → Einstellungen → Sprache
gewählt ist (`WCMD_DEU.LNG` → Deutsch, `WCMD_RUS.LNG` → Russisch, TCs
eingebautes Englisch → Englisch). `Language=deu` (oder ein anderer
Sektionsname) in `[Settings]` legt die Sprache unabhängig von TC fest.
Die Sprache wird einmal pro TC-Sitzung gelesen - nach einem
Sprachwechsel in TC also TC neu starten.

Die Texte kommen aus `RecentTab.lng` - eine Datei, eine Sektion
`[eng]`/`[deu]`/`[rus]` pro Sprache, wie bei TC-Plugins üblich -
gelesen mit dem eigenen UTF-8-sicheren Parser des Plugins statt mit
Windows' `GetPrivateProfileString` (das nicht-lateinische Schriften
über die ANSI-Codepage des Systems verhackstückelt, außer die Datei hat
eine UTF-16LE-BOM). Ausgeliefert mit `eng`, `deu`, `rus`, `ukr` und `dan`; jeder
fehlende oder leere Key fällt automatisch auf Englisch zurück. (Vor
1.5.1 hieß die Datei `RecentTab_lang.ini` - wird weiterhin gelesen,
falls keine `RecentTab.lng` existiert.)

Eine Sprache hinzufügen: die `[eng]`-Sektion kopieren, auf TCs
Drei-Buchstaben-Kürzel umbenennen (der Teil nach `WCMD_` im Namen von
TCs eigener Sprachdatei), die Werte übersetzen. Platzhalter wie
`%ls`/`%d`/`%lld` müssen gleich bleiben und in derselben Reihenfolge
stehen - weicht eine Zeile ab, wird für sie Englisch angezeigt, statt
einen Absturz zu riskieren. Keys mit `_one`/`_many` (und `_few` für
Sprachen wie Russisch) sind Einzahl-/Mehrzahlformen.

Eine Nebenwirkung, die man kennen sollte: Die `! ...`-Einträge im Panel
werden mit übersetzt. Ein TC-Button oder Hotkey, der einen davon beim
Namen anspricht (z. B. `\\RecentTab\! AUFN`), passt nur in dieser
Sprache - wer so etwas nutzt, legt `Language=` fest.
...
Alle elf Panel-Einträge liefern bereits ein eigenes, passendes Icon
...
Die vollständige Liste aller elf Schlüssel steht in der
...
mit zwei Feldern, **VON** und **BIS** - probier ruhig ein bisschen
...
- Uhrzeit vergessen? Bei VON wird der Tagesanfang angenommen, bei BIS
  das Tagesende - ein reines Datum in BIS deckt also den ganzen Tag ab.
- BIS einfach leer lassen, wenn's **bis jetzt** gehen soll.
...
### Ordnerbaum - sehen, wie sich die beobachteten Ordner tatsächlich verzweigen

`! menu` → `! Ordnerbaum` (oder Alt+Enter direkt auf `! menu`) öffnet
ein eigenständiges kleines Fenster
(gleicher optischer Stil wie das Suchfenster - dünne Ränder, eigenes
Theme, komplett losgelöst von TCs eigener Optik), das jeden
beobachteten Ordner als Baum zeigt - zusammen mit den Ordnern, in denen
die erfassten Dateien tatsächlich liegen, denn die liegen meist
mehrere Ebenen tiefer als der beobachtete Ordner selbst.

Ordner, die sich einen Elternordner teilen, werden automatisch darunter
gruppiert, auch wenn dieser Elternordner selbst nicht beobachtet wird -
reine visuelle Gruppierung, nichts zusätzlich erfasst. Eine Folge von
Ordnern, die jeweils nur genau ein Kind haben - kein echtes Verzweigen
unterwegs - wird zu einer einzigen Zeile zusammengefasst, statt fünf
oder sechs eingerückte Ebenen zu zeigen, die keine zusätzliche
Information tragen; liegen trotzdem Dateien direkt in einem dieser
Durchgangs-Ordner, bleibt die Zählung genau an diesem Namen sichtbar,
nicht durch die Zusammenfassung versteckt.

Ein beobachteter Ordner, dessen Pfad nicht mehr existiert, bekommt ein
eigenes, bewusst auffälliges Symbol (eine Straßensperre, nicht derselbe
Geist wie bei einzelnen verlorenen Dateien anderswo - ein ganzer
fehlender konfigurierter Ordner ist eine andere Situation als eine
einzelne verschwundene Datei).

Das Fenster wächst nicht über etwa eine Standard-Lister-Höhe hinaus (TC
gibt Plugins keine Möglichkeit, die tatsächliche Größe abzufragen, das
ist also eine Annäherung, keine echte Messung) - alles Höhere ist mit
dem Mausrad scrollbar. Siehe `notes/folder-tree/` für die ausführlichere
Begründung, inklusive warum dafür ausgerechnet GDI+ nötig war.
...
| `Language` | `auto` (Standard) übernimmt TCs Sprache; oder fester Sektionsname aus `RecentTab.lng` |
...
| `RecIcon` / `ResetIcon` / `RefreshIcon` / `MenuIcon` / `AgeIcon` / `BackIcon` / `AutoRefreshIcon` / `SortResetIcon` / `FolderTreeIcon` / `LostIcon` / `FallbackIcon` / `DeadPathIcon` | Icon-Überschreibungen pro Eintrag, alle optional |
...
- `RecentTab.lng` - Oberflächentexte in allen mitgelieferten Sprachen;
  muss im selben Ordner wie die Plugin-Datei liegen, da relativ zum
  eigenen Speicherort gesucht, nicht in TCs Konfigurationsordner
