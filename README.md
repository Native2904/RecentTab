...
**English** · [Deutsch](LIESMICH.md) · [Русский](README.ru.md) · [Українська](README.uk.md) · [Dansk](README.da.md)

A Total Commander WFX (File System) plugin that shows what you've been
working on: recently modified and newly created files from the folders
you care about, as one virtual panel.
...
- **Color themes** - four presets plus your own, each light/dark, or
...
- **Sort order reset** - one click undoes an accidental column-click
  sort
- **Folder tree** - `! Folder tree` in `! menu` (or Alt+Enter on
  `! menu`) shows how your watched
...
- No settings dialog: configuration happens by editing `RecentTab.ini`
  directly (the files in the list themselves are fully editable).
...
Language=auto        ; auto (default) = follow Total Commander's language / or fixed: eng / deu / rus / ...
...
RootButtons=          ; bring Reset;Refresh;Search;AutoRefresh;SortReset;FolderTree to the root, instead of just inside "! menu"
...
window, Lost-file info window, folder tree). Every role that carries meaning
...
By default RecentTab simply speaks Total Commander's language: whatever
you picked under Configuration → Options → Language (`WCMD_DEU.LNG` →
German, `WCMD_RUS.LNG` → Russian, TC's built-in English → English).
`Language=deu` (or any other section name) in `[Settings]` pins a
language regardless of TC. The language is read once per TC session -
after switching TC's language, restart TC.

Interface text comes from `RecentTab.lng` - one file, one `[eng]`/
`[deu]`/`[rus]` section per language, the usual TC plugin convention -
read with the plugin's own UTF-8-safe parser rather than Windows'
`GetPrivateProfileString` (which mangles non-Latin scripts through the
system's ANSI codepage unless the file has a UTF-16LE BOM). Ships with
`eng`, `deu`, `rus`, `ukr` and `dan`; any key left out or blank falls back to English
automatically. (Before 1.5.1 the file was called `RecentTab_lang.ini` -
still read if no `RecentTab.lng` exists.)

Adding a language: copy the `[eng]` section, rename it to TC's
three-letter code (the part after `WCMD_` in TC's own language file
name), translate the values. Placeholders like `%ls`/`%d`/`%lld` must
stay the same and in the same order - a line where they differ is
shown in English instead, rather than risking a crash. Keys ending in
`_one`/`_many` (and `_few` for Russian-style languages) are singular/
plural forms.

One side effect to know about: the `! ...` panel entries are
translated too. A TC button or hotkey that points at one of them by
name (e.g. `\\RecentTab\! REC`) only matches in that language - pin
`Language=` if you rely on such a shortcut.
...
All eleven panel entries ship with their own matching icon as an active
...
See Settings reference → Paths for the complete list of all eleven keys.
...
`! menu` → `! Folder tree` (or Alt+Enter on `! menu` itself) opens a
small standalone window (same visual
style as the search window - thin borders, its own theme, detached
from TC's own look) showing every watched folder as a tree - together
with the folders the recorded files actually sit in, since those are
usually several levels deeper than the watched folder itself.
...
| `Language` | `auto` (default) follows TC's language; or a fixed section name in `RecentTab.lng` |
...
| `RecIcon` / `ResetIcon` / `RefreshIcon` / `MenuIcon` / `AgeIcon` / `BackIcon` / `AutoRefreshIcon` / `SortResetIcon` / `FolderTreeIcon` / `LostIcon` / `FallbackIcon` / `DeadPathIcon` | Per-entry icon overrides, all optional |
...
- `RecentTab.lng` - interface text in all shipped languages; **must
  stay in the same folder as the plugin file**, since it's looked up
  relative to its own location, not TC's config folder
